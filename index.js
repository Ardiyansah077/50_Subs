/*
]=====> ARDIGANS YT <=====[ ]=====> PATRICK BOT <=====[ ]=====> 085282996146 <=====[
...
limitawal = 50
memberlimit = 4
cr = '*PATRICK 𝐁𝐎𝐓 𝐕𝐄𝐑𝐈𝐅𝐈𝐄𝐃*'

/*
]=====> SETTINGS <=====[
*/
const ownerNumber = ["6285282996146@s.whatsapp.net","62**********@s.whatsapp.net"] //nomerlu ganti yg *****
const premiumNumber =["6285282996146@s.whatsapp.net","62********@s.whatsapp.net"] //nomerlu ubah yg ***"*
const botName = 'PATRICK 𝐁𝐎𝐓'
const ownerName = 'PATRICK'
const premiumName = 'PATRICK'
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' SUBSCRIBE YT ARDIGANS YT'))
})
...
*/
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Teksnya mana kak? Contoh : ${prefix}nulis1 Ardi baik hati')
				ramlan = body.slice(8)
				reply('「❗」WAIT BRO  NGAK LAMA KOK :V')
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Teksnya mana kak? Contoh : ${prefix}nulis2 Ardi baik hati')
				laysha = body.slice(8)
				reply('「❗」WAIT BRO NGAK LAMA KOK :V')
				...
				client.sendMessage(from, ''+tes+'\n\n_By : ⸸Ardi⸸Panutanque._', text, { quoted: mek })
				await limitAdd(sender)
				break
// NINJALOGO				
				case 'ninjalogo':
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply('「❗」Contoh : ${prefix}ninjalogo Ardi & Gans')
				reply(ind.wait())
...
//GEMBOK TEXT
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}gemboktext 11 01 2021 & Ardi dan Dia')
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}glitchtext Ardi & Gans')
					reply(ind.wait())
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata Ardi`
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner Patrick`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena Patrick baik Bot memberikan ${one}Xp >_<`)
                      }else{
...
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply('「❗」Contoh : ${prefix}cloudtext Patrick')
cloud = body.slice(11)
